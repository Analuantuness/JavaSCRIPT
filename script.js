...
// Additional script for color filter if necessary
document.getElementById('color-filter').addEventListener('change', function(event) {
    const filterValue = event.target.value;
    document.querySelectorAll('.filter-image').forEach(img => {
        img.style.filter = getColorFilter(filterValue);
    });
});
